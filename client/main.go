...
func getFireSignalsChannel() chan os.Signal {
	...
	signal.Notify(c,
		...
		//syscall.SIGKILL, // TODO: SIGKILL and SIGSTOP cant be caught.
...
func main() {
	var (
		...
		username  = flag.String("username", os.Getenv("USER"), "Enter username for client")
	)
	flag.Parse()
	...
	go func() {
		<-exitChan
		cleanup(serverPID, clientPID, e) //not sure if we should pass the params as such
		os.Exit(1)
	}()
